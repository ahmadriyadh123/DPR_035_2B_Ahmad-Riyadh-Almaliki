...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeApp() {
        // Load komponen gaji data first time
        loadKomponen(1);
    }
    ...
    const getAddButton = () => {
        if (isUserAdmin()) {
            return '<button class="btn btn-primary add-komponen-btn">Tambah Komponen Gaji</button>';
        } else {
            ...
            komponen_gaji.forEach(komponen => {
                komponenGajiRows += `
                    <tr>
                        ...
                        <td>
                            ${getActionButtons(komponen.id_komponen_gaji, komponen.nama_komponen)}
                        </td>
                    </tr>`;
            });
        } else {
